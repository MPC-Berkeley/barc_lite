...
class PIDSteeringID(AbstractController):
    '''
    ...
    target speed: v_ref
    '''
    def __init__(self, v_ref: float, dt: float, u_steer: float,
                speed_pid_params: PIDParams = None):
        if speed_pid_params is None:
            speed_pid_params = PIDParams()
            speed_pid_params.dt = dt
    ...
    def step(self, vehicle_state: VehicleState, env_state = None):
        v = np.sqrt(vehicle_state.v.v_long**2 + vehicle_state.v.v_tran**2) # TODO velocity

        vehicle_state.u.u_a, _ = self.speed_pid.solve(v)
        vehicle_state.u.u_steer = self.u_steer
